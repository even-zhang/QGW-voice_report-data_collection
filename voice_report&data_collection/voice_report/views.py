...
def time_split(input_data):
    ...
    if se and (not hm):
	    times = re.sub(r'早上|早晨|上午', r'08:00', times)
    ...
    for i in range(0, size):
        for j in range(0, xsize):
            ...
            if tag == 'B':
                ...
                event.append(char)
    return ''.join(address), ''.join(event)

def data_split(input_data):
    try:
        tagger = CRFPP.Tagger("-m ../seg/model_new")
        input_data = re.sub(',|，|。', '', input_data)
        input_data, times = time_split(input_data)
        time1 = time.time()
        address, event = predict(input_data, tagger)
  
    except Exception as e:
        time2 = time.time()
        logging.warn(e + str(time2-time1))
        logging.error(e + str(time2-time1))
        logging.critical(e + str(time2-time1))
    address = del_time(address)
    return address, event, times
...
def index(request):
    if request.method == 'GET':
        ...
        try:
            ...
            address, event, time = data_split(input_data)
